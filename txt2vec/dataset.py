...
class Dataset(Dataset):
    def __init__(
        ...
        self.input_type = train_config['input']['type'] 
        self.energy_level_tag, *_ = get_variance_level(preprocess_config, model_config)

        self.basename, self.data_name, self.language, self.speaker, self.text = self.process_meta(
            filename
        )
        '''
        ...
        self.language_dict={'English':0,'French':1,'German':2,'Portuguese':3,'Spanish':4,'Swedish':5}
    ...
    def __getitem__(self, idx):
        ...
        ##mel_path, acutally means CodeIndex path
        ...
        #ldr = torch.load(ldr_path).cpu().numpy()
        spk_tdnn_emb = np.load(spk_path) 
        
        
        #print ('---DRs:',DR.shape)
        if self.input_type == 'XphoneBERT':
            text_path =  os.path.join(
            self.preprocessed_path,
            "xpbid_seq",
            "{}.npy".format(basename),
             )
            phone = np.load(text_path)[0]
            attn_prior_path = os.path.join(
                self.preprocessed_path,
                "xp_dr_prior",
                "{}-attn_prior-{}.npy".format('MM6', basename),
            )

        if self.input_type == 'Letters':
            text_path =  os.path.join(
            self.preprocessed_path,
            "letters_seq",
            "{}.npy".format(basename),
             )
            phone = np.load(text_path)
            attn_prior_path = os.path.join(
                self.preprocessed_path,
                "le_dr_prior",
                "{}-attn_prior-{}.npy".format('MM6', basename),
    ...
    def process_meta(self, filename):
        with open(
            ...
            return name, data, language, speaker, text
    def reprocess(self, data, idxs):
        ids = [data[idx]["id"] for idx in idxs]
        speakers = [data[idx]["speaker"] for idx in idxs]
        texts = [data[idx]["text"] for idx in idxs]
        language = [data[idx]["spker_embed"] for idx in idxs]
        raw_texts = [data[idx]["raw_text"] for idx in idxs]
        mels = [data[idx]["mel"] for idx in idxs]
        DRs = [data[idx]["DR"] for idx in idxs]
        ...
        #print ('---mels:',np.array(mels).shape)
        ...
        DRS = pad_2D(DRs)
        '''
        mels_1 = F.one_hot(torch.tensor(mels[:,:,0]),320)
        mels_2 = F.one_hot(torch.tensor(mels[:,:,1]),320)
        mels = torch.cat([mels_1,mels_2],-1)
        '''
        #print (mels)
        mels = np.array(mels)
        #DRs =  np.array(DRs)
        ...
        #print ('---after pad mels---:',mels.shape)
        #print ('---mel2phs----!!!',mel2phs)
        return (
            ids,
            raw_texts,
            speakers,
