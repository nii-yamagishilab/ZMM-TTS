...
class CompTransTTSLoss(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config, train_config):
        ...
        book1 = torch.from_numpy(np.load('Dataset/MM6/code_book1.npy')).unsqueeze(0)
        book2 = torch.from_numpy(np.load('Dataset/MM6/code_book2.npy')).unsqueeze(0)
        self.code_book1 = book1.cuda()
        self.code_book2 = book2.cuda()
        self.code_book1.requires_grad = False
        self.code_book2.requires_grad = False
        
        #self.code_book1 = self.code_book.detach()
    ...
    def get_mel_loss(self, mel_predictions, mel_targets):
        ...
        #print (mel_targets[:,:,0].shape)
        #print (mel_targets[:,:,0].unsqueeze(-1).shape)
        #print (mel_predictions[:,:,:320].shape)
        #print (mel_predictions[:,:,:320].shape)
        code_loss1 = torch.nn.CrossEntropyLoss()(mel_predictions[:,:,:320].permute(0,2,1), mel_targets[:,:,0].long())
        code_loss2 = torch.nn.CrossEntropyLoss()(mel_predictions[:,:,320:].permute(0,2,1), mel_targets[:,:,1].long())
        #code_loss1 = torch.nn.NLLLoss()(mel_predictions[:,:,:320].permute(0,2,1), mel_targets[:,:,0].long())
        #code_loss2 = torch.nn.NLLLoss()(mel_predictions[:,:,320:].permute(0,2,1), mel_targets[:,:,1].long())
        mel_loss = code_loss1 + code_loss2
        return mel_loss 
    ...
    def get_mel_loss(self, mel_predictions, mel_targets):
        mel_targets.requires_grad = False
        mel_predictions = mel_predictions.view(mel_predictions.shape[0],mel_predictions.shape[1],640) 
        mel_predictions = mel_predictions.masked_fill(self.mel_masks_fill.unsqueeze(-1), 0)
        mel_targets = mel_targets.masked_fill(self.mel_masks_fill.unsqueeze(-1), 0)
        #print (mel_targets[:,:,0].shape)
        #print (mel_targets[:,:,0].unsqueeze(-1).shape)
    ...
    def get_glumb_loss(self, dr_pred, dr_targets, temp, code_targets):
        dr_targets.requires_grad = False
        code_targets.requires_grad = False
        #dr_predictions = mel_predictions.masked_fill(self.mel_masks_fill.unsqueeze(-1), 0)
        dr_targets = dr_targets.masked_fill(self.mel_masks_fill.unsqueeze(-1), 0)
        
        #output = F.gumbel_softmax(dr_pred, tau=temp, hard=True)
        output = F.softmax(dr_pred,dim=-1)
        
        
        batch_size = output.shape[0]
        sequence_length = output.shape[1]
        output_fast1 = output[:,:,0,:].squeeze(-2)
        output_fast2 = output[:,:,1,:].squeeze(-2)
        codebook_fast1 = self.code_book1.repeat([batch_size, 1, 1])
        codebook_fast2 = self.code_book2.repeat([batch_size, 1, 1])
        codevectors_fast1 = torch.bmm(output_fast1, codebook_fast1)
        codevectors_fast2 = torch.bmm(output_fast2, codebook_fast2)
        codevectors_fast = torch.cat([codevectors_fast1, codevectors_fast2], dim=-1) 
        
        
        
        #print ('output',output.shape)
        #print ('output2',output_softmax.shape)
        #print ('------',output[0,0,0])
        #print ('111111',output[0,0,1])
        #print ('----outpu---shape',output.shape)
        batch_size = output.shape[0]
        sequence_length = output.shape[1]
        mel_targets_1 = code_targets.masked_fill(self.mel_masks_fill.unsqueeze(-1), 0)
        code_loss = nn.functional.nll_loss(torch.log10(output+0.00000001).permute(0,3,1,2).contiguous(), mel_targets_1)
        ...
        #print ('xxxxxxxx',self.code_book.shape)
        #print ('22222222',output.unsqueeze(-1).shape)
        #output1= output.unsqueeze(-1) * self.code_book
        #print ('output1--shape',output1.shape)
        #print (output1[0,0,0,146])
        #print ('---second-----')
        #print (self.code_book[1,102])
        #print (output1[0,0,1,102])
        #codevectors = output1.view(batch_size * sequence_length, 2, 320, -1)
        #print ('---sum----',output2)
        #codevectors = codevectors.sum(-2).view(batch_size, sequence_length, -1) 
        #print (codevectors.shape)
        dr_predictions = codevectors_fast.masked_fill(self.mel_masks_fill.unsqueeze(-1), 0)
        #print ('---final----',output3)
        #print ('------',output3.shape)
        #print ('------',output3[0,0]) 
        #print ('---dr--target',dr_targets.shape)
        #print (dr_predictions.shape)
        dr_loss = self.l1_loss(dr_predictions, dr_targets)        
        
        return dr_loss, code_loss
    ...
    def forward(self, inputs, predictions, step):
        ...
        DR_targets = DR_targets[:, : mel_masks.shape[1], :]
        ...
        #print ('-----',DR_targets.shape)
        #print (step)
        delay_step = 600000-step
        curr_temp = max(2.0-2.0*np.power(0.5,delay_step), 1.0)
        #print (curr_temp)
        postnet_mel_loss, mel_loss = self.get_glumb_loss(postnet_mel_predictions,DR_targets,curr_temp, mel_targets)
        #self.get_mel_loss(postnet_mel_predictions, mel_targets)
