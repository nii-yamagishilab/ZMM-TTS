...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print ('---spk_tdnn_emb----:',spk_tdnn_emb)
    ...
    def reprocess(self, data, idxs):
        ...
        ldrs = [data[idx]["ldr"] for idx in idxs]
        #print ('ldrs-----',ldrs)
        ...
        ldr_lens = np.array([ldr.shape[0] for ldr in ldrs])
        speakers = np.array(speakers)
        #print ('00000000',speakers.shape)
        ...
        if self.learn_alignment:
            #attn_priors = pad_3D(attn_priors, len(idxs), max(ldr_lens), max(mel_lens))
            attn_priors = None
        else:
            durations = pad_1D(durations)
            mel2phs = pad_1D(mel2phs)
        #print ('-----ldrs2-----',ldrs)
        #print ('-----ldrs2 shapespe-----',ldrs.shape)
        #print ('-----ldrs2 dtype-----',ldrs.dtype)
        return (
            ids,
            raw_texts,
            speakers,
    ...
    def collate_fn(self, data):
        ...
        if self.sort:
            len_arr = np.array([d["ldr"].shape[0] for d in data])
